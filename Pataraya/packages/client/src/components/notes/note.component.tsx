...
// Функция onDeleteHandler будет вызывать deleteNote RPC в rpc API, чтобы удалить запись, соответствующую запросу, из БД
const NoteItem: FC<NoteItemProps> = ({ note }) => {
    ...
    const { mutate: deleteNote } = trpc.deleteNote.useMutation({
        onSuccess() {
            // После удаления заметки, будет вызвана getNotes (вернет первые 10 записей из БД)
            ...
            {format(parseISO(String(note.createdAt)), "PPP")}
                    ...
                    >
                        <ul className="py-1" aria-labelledby="dropdownButton">
                            ...
                            >
                                <i className="bx bx-pencil"></i> Редактировать
                            </li>
                            <li
                                onClick={() => {
                                    setOpenSettings(false);
